...
const ANCHOR_HEIGHT = 18 // 每个元素定义的高度
...
export default {
  ...
  methods: {
    ...
    onShortcutTouchMove(e) {
      ...
      let delta = (this.touch.y2 - this.touch.y1) / ANCHOR_HEIGHT | 0 //   |0向下取整,运算规则(有1则为1)  相当于Math.floor()
    ...
    // 私有方法公用方法放下面 绑定事件一般放上面
    _scrollTo(index) {
      if (!index && index !== 0) {
        return // 不需要改变状态就return 出去
      }
      if (index < 0) {
        index = 0
